...
func detectDebian(c config.ServerInfo) (itsMe bool, deb osTypeInterface, err error) {
	...
	if r := runcmd(c, "ls /etc/debian_version", noSudo); !r.isSuccess() {
		if r.Error != nil {
	...
	if r := runcmd(c, "lsb_release -ir", noSudo); r.isSuccess() {
		//  e.g.
	...
	if r := runcmd(c, "cat /etc/lsb-release", noSudo); r.isSuccess() {
		//  e.g.
		//  DISTRIB_ID=Ubuntu
	...
	if r := runcmd(c, cmd, noSudo); r.isSuccess() {
...
func (o *debian) scanInstalledPackages() (packs []models.PackageInfo, err error) {
	r := o.runcmd("dpkg-query -W", noSudo)
	if !r.isSuccess() {
...
func (o *debian) checkRequiredPackagesInstalled() error {
	if o.Distro.Family == "debian" {
		if r := o.runcmd("test -f /usr/bin/aptitude", noSudo); !r.isSuccess() {
