...
func detectRedhat(c config.ServerInfo) (itsMe bool, red osTypeInterface) {
	...
	if r := runcmd(c, "ls /etc/fedora-release", noSudo); r.isSuccess() {
		red.setDistro("fedora", "unknown")
		Log.Warn("Fedora not tested yet: %s", r)
		return true, red
	}

	if r := runcmd(c, "ls /etc/redhat-release", noSudo); r.isSuccess() {
		// https://www.rackaid.com/blog/how-to-determine-centos-or-red-hat-version/
		// e.g.
		// $ cat /etc/redhat-release
		// CentOS release 6.5 (Final)
		if r := runcmd(c, "cat /etc/redhat-release", noSudo); r.isSuccess() {
	...
	if r := runcmd(c, "ls /etc/system-release", noSudo); r.isSuccess() {
		family := "amazon"
		release := "unknown"
		if r := runcmd(c, "cat /etc/system-release", noSudo); r.isSuccess() {
...
func (o *redhat) checkDependencies() error {
	...
	case "centos":
		...
		cmd := "rpm -q " + name
		if r := o.runcmd(cmd, noSudo); r.isSuccess() {
			return nil
		}
...
func (o *redhat) install() error {
	for _, name := range o.lackDependencies {
		...
		if r := o.runcmd(cmd, sudo); !r.isSuccess() {
...
func (o *redhat) scanUnsecurePackagesUsingYumCheckUpdate() (models.VulnInfos, error) {
	...
	r := o.runcmd(util.PrependProxyEnv(cmd), sudo)
